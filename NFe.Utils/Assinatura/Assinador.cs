...
namespace NFe.Utils.Assinatura
{
    public static class Assinador
    {
        ...
        /// <param name="configuracaoServico">Configuração do serviço</param>
        /// <returns>Retorna um objeto do tipo Classes.Assinatura.Signature, contendo a assinatura do objeto passado como parâmetro</returns>
        public async static Task<Signature> ObterAssinaturaAsync<T>(T objeto, string id, ConfiguracaoServico configuracaoServico = null, CancellationToken cancellationToken = default) where T : class
        {
            var cfgServico = configuracaoServico ?? ConfiguracaoServico.Instancia;
            ...
            {
                certificadoDigital = CertificadoDigital.ObterCertificado(cfgServico.Certificado);
                return await ObterAssinaturaAsync<T>(objeto, id, certificadoDigital, cfgServico.Certificado.ManterDadosEmCache, cfgServico.Certificado.SignatureMethodSignedXml, cfgServico.Certificado.DigestMethodReference, cfgServico.RemoverAcentos, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                if (!cfgServico.Certificado.ManterDadosEmCache)
        ...
        /// <param name="cfgServicoRemoverAcentos"></param>
        /// <returns>Retorna um objeto do tipo Classes.Assinatura.Signature, contendo a assinatura do objeto passado como parâmetro</returns>
        public async static Task<Signature> ObterAssinaturaAsync<T>(T objeto, string id, X509Certificate2 certificadoDigital,
            bool manterDadosEmCache = false, string signatureMethod = "http://www.w3.org/2000/09/xmldsig#rsa-sha1",
            string digestMethod = "http://www.w3.org/2000/09/xmldsig#sha1", bool cfgServicoRemoverAcentos = false, CancellationToken cancellationToken = default) where T : class
        {
            var objetoLocal = objeto;
            if (id == null)
                throw new Exception("Não é possível assinar um objeto evento sem sua respectiva Id!");
            ...
            try
            {
                var documento = new XmlDocument { PreserveWhitespace = true };
                var xml = cfgServicoRemoverAcentos
                    ? (await FuncoesXml.ClasseParaXmlStringAsync(objetoLocal, cancellationToken).ConfigureAwait(false))?.RemoverAcentos()
                    : await FuncoesXml.ClasseParaXmlStringAsync(objetoLocal, cancellationToken).ConfigureAwait(false);
